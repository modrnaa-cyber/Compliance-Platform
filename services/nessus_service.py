...
class NessusService:
    def __init__(self, base_path: str = "data/nessus"):
    ...
    def extract_findings(self, scan_data: Dict[str, Any], target: str = None) -> List[Dict[str, Any]]:
        findings = []

        raw_findings = scan_data.get("findings", [])
        scan_target = scan_data.get("scan_target", target)

        for item in raw_findings:
            host = self._safe_get(item, "host", scan_target)
            port = self._safe_get(item, "port")
            protocol = str(self._safe_get(item, "protocol", "tcp")).lower()
